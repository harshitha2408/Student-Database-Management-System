...
def export_data():
    ...
    table=pandas.DataFrame(newlist,columns=['Id','Name','Email','Address','Phone','Gender','DOB','Added Time','Added Date'])
...
def update_data():
    ...
    mycursor.execute(query,(nameEntry.get(),phoneEntry.get(),emailEntry.get(),addressEntry.get(),dobEntry.get(),genderEntry.get(),date,currenttime,idEntry.get()))
    con.commit()
...
def  search_data():
    query='select * from student where id=%s or name=%s or email=%s or address=%s or gender=%s or dob=%s'
    mycursor.execute(query,(idEntry.get(),nameEntry.get(),emailEntry.get(),addressEntry.get(),genderEntry.get(),dobEntry.get()))
    studentTable.delete(*studentTable.get_children())
    fetched_data=mycursor.fetchall()
    for data in fetched_data:
        studentTable.insert('',END,values=data)
...
def connect_database():
    def connect():
        global mycursor,con
        try:
            con = pymysql.connect(host='localhost', user='root', password='harshi')
            mycursor = con.cursor()
...
def clock():
    ...
    # Schedule the clock function to be called after 1 second (1000 milliseconds)
...
datetimeLabel = Label(root, font=("Arial", 14, "bold"))
...
style=ttk.Style()
style.configure('Treeview',rowheight=40,font=('arial',12,'bold'),foreground='red4',background='aqua',fieldbackground='blue')
